...
const SS2022_CONTEXT = enc.encode('shadowsocks 2022 session subkey');
...
function readU16be(b: Uint8Array, off = 0): number {
  return (b[off] << 8) | b[off + 1];
}

function u64be(n: number): Uint8Array {
  const out = new Uint8Array(8);
  new DataView(out.buffer).setBigUint64(0, BigInt(Math.floor(n)), false);
  return out;
}

function readU64be(b: Uint8Array, off = 0): number {
...
function nextNonce(nonce: Uint8Array): Uint8Array {
...
function addressHeader(host: string, port: number): Uint8Array {
  const h = enc.encode(host);
  if (h.length > 255) throw new Error('Shadowsocks target hostname is too long');
  return concat(new Uint8Array([0x03, h.length]), h, u16be(port));
}

function evpBytesToKey(password: string, keyLen: number): Uint8Array {
  ...
  while (out.length < keyLen) {
    ...
    h.update(p);
    prev = Uint8Array.from(h.digest());
    out = concat(out, prev);
  }
  return out.slice(0, keyLen);
}

function decode2022Psk(value: string, keyLen: number): Uint8Array {
  let bin: string;
  try { bin = atob(value.trim()); } catch { throw new Error('Shadowsocks 2022 password must be a Base64 PSK'); }
...
function deriveSubkey(master: Uint8Array, salt: Uint8Array, info: MethodInfo): Uint8Array {
  if (info.is2022) {
    return blake3(concat(master, salt), { context: SS2022_CONTEXT, dkLen: info.keyLen });
  }
  return Uint8Array.from(hkdfSync('sha1', master, salt, enc.encode('ss-subkey'), info.keyLen) as unknown as ArrayLike<number>);
}

function equalTag(a: Uint8Array, b: Uint8Array): boolean {
...
function chachaBlock(key: Uint8Array, nonce: Uint8Array, counter: number, rounds: number): Uint8Array {
  ...
  initial.set(constants, 0);
  for (let i = 0; i < 8; i++) initial[4 + i] = kd.getUint32(i * 4, true);
  initial[12] = counter >>> 0;
  initial[13] = nd.getUint32(0, true); initial[14] = nd.getUint32(4, true); initial[15] = nd.getUint32(8, true);
...
function chachaXor(key: Uint8Array, nonce: Uint8Array, input: Uint8Array, rounds: number, counter = 1): Uint8Array {
  const out = new Uint8Array(input.length);
  for (let off = 0; off < input.length; off += 64, counter++) {
    const block = chachaBlock(key, nonce, counter, rounds);
...
function bigIntLe(n: bigint, len: number): Uint8Array {
  const out = new Uint8Array(len);
...
function poly1305(message: Uint8Array, key: Uint8Array): Uint8Array {
  ...
  const r = leBigInt(rBytes), s = leBigInt(key.slice(16, 32));
  const p = (1n << 130n) - 5n;
  let acc = 0n;
  for (let off = 0; off < message.length; off += 16) {
    const block = message.slice(off, Math.min(off + 16, message.length));
    const n = leBigInt(block) + (1n << BigInt(block.length * 8));
    acc = ((acc + n) * r) % p;
  }
  return bigIntLe((acc + s) & ((1n << 128n) - 1n), 16);
}

function pad16(x: Uint8Array): Uint8Array {
  const n = (16 - (x.length % 16)) % 16;
  return n ? concat(x, new Uint8Array(n)) : x;
}

function chacha8Seal(key: Uint8Array, nonce: Uint8Array, plain: Uint8Array): Uint8Array {
  const polyKey = chachaBlock(key, nonce, 0, 8).slice(0, 32);
  const ct = chachaXor(key, nonce, plain, 8, 1);
  const tag = poly1305(concat(pad16(new Uint8Array(0)), pad16(ct), u64le(0), u64le(ct.length)), polyKey);
  return concat(ct, tag);
}

function chacha8Open(key: Uint8Array, nonce: Uint8Array, data: Uint8Array): Uint8Array {
  if (data.length < TAG_LEN) throw new Error('Shadowsocks chacha8 ciphertext is truncated');
  const ct = data.slice(0, -TAG_LEN), tag = data.slice(-TAG_LEN);
  const polyKey = chachaBlock(key, nonce, 0, 8).slice(0, 32);
  const expected = poly1305(concat(pad16(new Uint8Array(0)), pad16(ct), u64le(0), u64le(ct.length)), polyKey);
  if (!equalTag(tag, expected)) throw new Error('Shadowsocks chacha8 authentication failed');
  return chachaXor(key, nonce, ct, 8, 1);
}

function seal(info: MethodInfo, key: Uint8Array, nonce: Uint8Array, plain: Uint8Array): Uint8Array {
  if (info.cipher === 'chacha8-poly1305') return chacha8Seal(key, nonce, plain);
  const algorithm = info.cipher;
  const cipher = createCipheriv(algorithm, key, nonce, { authTagLength: TAG_LEN });
  const body = concat(Uint8Array.from(cipher.update(plain)), Uint8Array.from(cipher.final()));
  return concat(body, Uint8Array.from(cipher.getAuthTag()));
}

function open(info: MethodInfo, key: Uint8Array, nonce: Uint8Array, data: Uint8Array): Uint8Array {
  ...
  const decipher = createDecipheriv(info.cipher, key, nonce, { authTagLength: TAG_LEN });
...
class ShadowsocksDuplex extends Duplex {
  ...
  private reqNonce = new Uint8Array(12);
  private respNonce = new Uint8Array(12);
  private reqSalt: Uint8Array;
  private reqKey: Uint8Array;
  private respKey: Uint8Array | null = null;
  ...
  constructor(
    ...
    super();
    this.reqSalt = Uint8Array.from(randomBytes(info.saltLen));
    this.reqKey = deriveSubkey(masterKey, this.reqSalt, info);
    ...
    socket.once('close', () => { if (!this.destroyed) this.push(null); });
    socket.once('error', err => this.destroy(err));
  }

  _read(): void {}

  private writeRaw(data: Uint8Array, cb: (error?: Error | null) => void) {
    this.socket.write(data, err => cb(err || undefined));
  }

  private framed(data: Uint8Array): Uint8Array {
    const parts: Uint8Array[] = [];
    for (let off = 0; off < data.length; off += MAX_CHUNK) {
      const chunk = data.slice(off, off + MAX_CHUNK);
      parts.push(seal(this.info, this.reqKey, this.reqNonce, u16be(chunk.length)));
      this.reqNonce = nextNonce(this.reqNonce);
      parts.push(seal(this.info, this.reqKey, this.reqNonce, chunk));
      this.reqNonce = nextNonce(this.reqNonce);
    }
    return concat(...parts);
  }
  ...
  _write(chunk: Uint8Array, _encoding: BufferEncoding, cb: (error?: Error | null) => void): void {
    try {
      ...
      const addr = addressHeader(this.targetHost, this.targetPort);

      if (!this.info.is2022) {
        const firstRoom = Math.max(0, MAX_CHUNK - addr.length);
        const firstPayload = concat(addr, data.slice(0, firstRoom));
        const first = concat(this.reqSalt, this.framed(firstPayload), this.framed(data.slice(firstRoom)));
        this.writeRaw(first, cb);
        return;
      }

      const firstPayload = data.slice(0, MAX_CHUNK);
      const variable = concat(addr, u16be(0), firstPayload);
      ...
      const rest = data.slice(firstPayload.length);
      const packet = concat(this.reqSalt, c1, c2, rest.length ? this.framed(rest) : new Uint8Array(0));
      this.writeRaw(packet, cb);
  ...
  private take(n: number): Uint8Array {
    ...
    this.incoming = this.incoming.slice(n);
    return out;
  }
    ...
    try {
      while (true) {
        if (!this.respKey) {
          ...
          const salt = this.take(this.info.saltLen);
          this.respKey = deriveSubkey(this.masterKey, salt, this.info);
        }
        ...
        if (this.info.is2022 && !this.responseHeaderDone) {
          ...
          const fixed = open(this.info, this.respKey, this.respNonce, this.take(fixedLen + TAG_LEN));
          this.respNonce = nextNonce(this.respNonce);
          if (fixed[0] !== 1) throw new Error('Invalid Shadowsocks 2022 response header type');
          const ts = readU64be(fixed, 1);
          if (Math.abs(Date.now() / 1000 - ts) > 30) throw new Error('Shadowsocks 2022 response timestamp is outside the allowed window');
          const reqSalt = fixed.slice(9, 9 + this.info.saltLen);
          if (!equalTag(reqSalt, this.reqSalt)) throw new Error('Shadowsocks 2022 response does not match request salt');
          this.responseInitialLen = readU16be(fixed, 9 + this.info.saltLen);
          this.responseHeaderDone = true;
          if (this.responseInitialLen === 0) continue;
        }

        if (this.info.is2022 && this.responseInitialLen !== null) {
          const n = this.responseInitialLen;
          if (this.incoming.length < n + TAG_LEN) return;
          const plain = open(this.info, this.respKey, this.respNonce, this.take(n + TAG_LEN));
          this.respNonce = nextNonce(this.respNonce);
          this.responseInitialLen = null;
        ...
        if (this.responsePayloadLen === null) {
          ...
          const lenPlain = open(this.info, this.respKey, this.respNonce, this.take(2 + TAG_LEN));
          this.respNonce = nextNonce(this.respNonce);
          this.responsePayloadLen = readU16be(lenPlain);
        }

        const n = this.responsePayloadLen;
        if (this.incoming.length < n + TAG_LEN) return;
        const plain = open(this.info, this.respKey, this.respNonce, this.take(n + TAG_LEN));
        this.respNonce = nextNonce(this.respNonce);
        this.responsePayloadLen = null;
        if (plain.length) this.push(plain);
      }
    } catch (err) {
...
export async function openShadowsocksTunnel(targetHost: string, targetPort: number, proxy: ProxyConfig): Promise<Duplex> {
  ...
  const info = METHODS[proxy.ssMethod];
  if (!info) throw new Error(`Unsupported Shadowsocks method: ${proxy.ssMethod}`);
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    const cleanup = () => { socket.off('connect', done); socket.off('error', fail); };
    socket.once('connect', done); socket.once('error', fail);
  });
