...
async function loadInbox() {
  ...
  try {
    ...
    if (data.failures?.length) { banner.textContent = `${data.failures.length} 个邮箱连接失败：` + data.failures.map(f => state.accounts.find(a => a.id === f.accountId)?.label || f.accountId).join('、'); banner.classList.remove('hidden'); }
